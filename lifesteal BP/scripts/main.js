...
const { keys: settingsKey, settingsMapping } = flattenKeys(defaultAddonSetting);
...
Mc.world.afterEvents.playerSpawn.subscribe((eventData) => {
    if (eventData.initialSpawn === true) {
        ...
        if ((getPlayersMaxHealth(eventData.player)/2) > max) setPlayersHealth({ player: eventData.player, hearts: max });
    }
...
Mc.system.beforeEvents.startup.subscribe((eventData)=>{
    settingsKey[0]=Mc.CustomCommandParamType.Integer;
    ...
    eventData.customCommandRegistry.registerCommand({
      ...
      description: "Allows players to withdraw hearts as a pysical item form through a gui.",
      permissionLevel: Mc.CommandPermissionLevel.Any,
      cheatsRequired: false
    },    
    (data) => {
        Mc.system.run(()=>{
        if(getAddonSetting("withdrawCommand")===false){
            data.sourceEntity.sendMessage(`§cThis command was disabled by an admin!`);
            return {status: Mc.CustomCommandStatus.Failure, message: `§cThis command was disabled by an admin!`};
        };
        if(data.sourceType!=="Entity"||data.sourceEntity.typeId!=="minecraft:player") return {status: Mc.CustomCommandStatus.Failure, message: "§aOnly players may withdraw hearts!"};
        const max = getPlayersMaxHealth(data.sourceEntity);
        if ((max / 2) < 2) {
            data.sourceEntity.sendMessage(`§cInsufficient amount of hearts gain more hearts to withdraw hearts!`);
            return;
        };
        openForm({ player: data.sourceEntity, formKey: "withdrawHearts", admin: max });
        return {status: Mc.CustomCommandStatus.Success, message: `§aGui opened.`};
        });
    });
    eventData.customCommandRegistry.registerCommand({
    name: "lifesteal:withdraw", 
      description: "Allows players to withdraw hearts as a pysical item form.",
      permissionLevel: Mc.CommandPermissionLevel.Any,
      cheatsRequired: false, 
    ...
    (data, parmas) => {
        ...
        let amount = 1;
        if(parmas!==undefined) amount=parmas;
        if(data.sourceType!=="Entity"||data.sourceEntity.typeId!=="minecraft:player") return {status: Mc.CustomCommandStatus.Failure, message: "§aOnly players may withdraw hearts!"};
        let max = getAddonSetting("maxHealth");
        if(amount>=max||amount<=0) {
            data.sourceEntity.sendMessage(`§cInsufficient number of hearts specified! [1-${max-1}]`);
            return {status: Mc.CustomCommandStatus.Failure, message: `§cInsufficient number of hearts specified! [1-${max-1}]`};
        };
        let getmaxValue = getPlayersMaxHealth(data.sourceEntity)/2;
        if (getmaxValue - amount < 1) {
            data.sourceEntity.sendMessage(`§cInsufficient amount of hearts gain more hearts to withdraw §7${amount} §chearts!`);
            return {status: Mc.CustomCommandStatus.Failure, message: `§cInsufficient amount of hearts gain more hearts to withdraw §7${amount} §chearts!`};
        };
        let event = "unitx:health" + (getmaxValue - amount);
        data.sourceEntity.triggerEvent(event);
        data.sourceEntity.getComponent("minecraft:inventory").container.addItem(new Mc.ItemStack("unitx:heart",amount));
        data.sourceEntity.sendMessage(`§aYou have withdrawn §7${amount} §aheart(s)`);
        return {status: Mc.CustomCommandStatus.Success, message: `§aYou have withdrawn §7${amount} §aheart(s)`};
        });
    })
...
export async function openForm({ player, formKey, admin, location }) {
    ...
    if (formConfig.type === "actionFormData") {
        ...
        for (const [value, field] of Object.entries(elements)) {
            let settings = {};
            if(field.tooltip!==undefined) settings.tooltip=field.tooltip;
            if (field.id === "slider") {//slider
                ...
                settings.valueStep = field.step; settings.defaultValue=defaultValue;
                form.slider(field.name, field.min, max, settings);
                ...
                settings.defaultValueIndex = defaultValue;
                form.dropdown(field.name, dropdownOptions, settings);
                ...
                settings.defaultValue = toggleValue;
                form.toggle(field.name, settings);
                ...
                settings.defaultValue = data;
                form.textField(field.name, "",settings);
                count++;
...
function flattenKeys(obj, parentKey = "", mapping = {}, keys = []) {
    for (const [key, value] of Object.entries(obj)) {
        const newKey = parentKey ? `${parentKey}.${toSnakeCase(key)}` : toSnakeCase(key);

        if (typeof value === "object" && !Array.isArray(value)) {
            flattenKeys(value, newKey, mapping, keys);
        } else if (Array.isArray(value)) {
            value.forEach((v, i) => {
                const arrKey = `${newKey}.${i + 1}`;
                keys.push(arrKey);
                mapping[arrKey] = `${parentKey}.${key}[${i}]`;
            });
        } else {
            keys.push(newKey);
            mapping[newKey] = parentKey ? `${parentKey}.${key}` : key;
        };
    };
    return { keys, mapping };
};